            ...
            if (req.body.title) {
                ...
                if (title != "") {
                    await new Promise((resolve, reject) => {
                        db.query('SELECT title FROM catalog WHERE user_id=' + user_id + ' AND title="' + title + '"', async (error, results) => {
                            if (error) {
                                throw error
                            }
                            if (results.length == 0) {

                                db.query('INSERT INTO catalog SET ?', {
                                    ...
                                    for (i = 0; i < total_sections; i++) {
                                        ...
                                        await new Promise((resolve, reject) => {
                                            db.query('INSERT INTO catalog_section SET ?', {
                                                catalog_key: catalog_key,
                                                catalog_section_key: catalog_section_key,
                                                user_id: user_id,
                                                catalog_section_number: catalog_section_number,
                                                catalog_section_title: catalog_section_title,
                                                catalog_section_status: catalog_section_status
                                            }, async (error, results) => {
                                                if (error) {
                                                    reject(error);
                                                    // req.flash("error","Failed to Created!")
                                                ...
                                                for (var j = 0; j < total_questions; j++) {
                                                    ...
                                                    if (catalog_questions_num != '') {
                                                        ...
                                                        await new Promise((resolve, reject) => {
                                                            db.query('INSERT INTO catalog_questions SET ?', {
                                                                catalog_key: catalog_key,
                                                                catalog_section_key: catalog_section_key,
                                                                catalog_questions_key: catalog_questions_key,
                                                                user_id: user_id,
                                                                catalog_section_id: catalog_section_id,
                                                                catalog_questions_num: catalog_questions_num,
                                                                catalog_questions_name: catalog_questions_name,
                                                                catalog_questions_required: catalog_questions_required,
                                                                catalog_questions_toggle: catalog_questions_toggle
                                                            }, async (error, results) => {
                                                                if (error) {
                                                                    reject(error);
                                                                    // req.flash("error","Failed to Created!")
                               ...
                               sessionStorage.setItem("error", "Title Already Exists!");
                                req.flash("error", "Catalog title already exists")
    ...
    updatecatalog: function(req, res) {
        ...
        if (req.user) {
            if (req.body.name) {
                ...
                db.query('SELECT title FROM catalog WHERE user_id=' + user_id + ' AND title="' + value + '"', async (error, results) => {
                    if (error) {
                        res.send({
                            status: "error",
                            name: name,
        ...
        if (req.user) {
            ...
            for (i = 1; i <= req.body.sectioncount.length; i++) {
                ...
                if (parseInt(sec_count.count) > 0) {
                    await new Promise((resolve, reject) => {
                        db.query('UPDATE catalog_section SET ? WHERE catalog_key=? AND user_id=? AND catalog_section_number=?', [{
                            catalog_section_title: catalog_section_title,
                            catalog_section_status: catalog_section_status
                        }, catalog_key, user_id, catalog_section_number], async (error, results) => {
                            if (error) {
                                reject(error);
                                // req.flash("error","Failed to Created!")
                                // res.redirect("/")
                            ...
                            var questionsrequired = req.body.questionsrequired;
                            console.log(req.body.questionscount[i])
                            if (req.body.questionscount[i] != undefined && req.body.questionscount[i] != "") {
                                for (var j = 1; j <= req.body.questionscount[i].length; j++) {
                                    ...
                                    if (catalog_questions_num != '' && catalog_questions_num > 0) {
                                        ...
                                        await new Promise((resolve, reject) => {
                                            console.log("==============++++===========" + ques_count.count)
                                            if (parseInt(ques_count.count) > 0) {
                                                db.query('UPDATE catalog_questions SET ? WHERE catalog_key=? AND user_id=? AND catalog_questions_num=?', [{
                                                    catalog_questions_name: catalog_questions_name,
                                                    catalog_questions_required: catalog_questions_required,
                                                    catalog_questions_toggle: catalog_questions_toggle
                                                }, catalog_key, user_id, catalog_questions_num], async (error, results) => {
                                                    if (error) {
                                                        reject(error);
                                                        // req.flash("error","Failed to Created!")
                                                        // res.redirect("/")
                                                ...
                                                catalog_section_key = req.body.sectionkey[i];
                                                console.log("---------+++++---------" + catalog_section_key)
                                                db.query('INSERT INTO catalog_questions SET ?', {
                                                    catalog_key: catalog_key,
                                                    catalog_section_key: catalog_section_key,
                                                    catalog_questions_key: catalog_questions_key,
...
async function getCountQuestions(questionscount, catalog_key, user_id) {
    return new Promise((resolve, reject) => {
        console.log('SELECT count(*) as count FROM catalog_questions WHERE user_id=' + user_id + ' and catalog_questions_num = "' + questionscount + '" and catalog_key="' + catalog_key + '"')
